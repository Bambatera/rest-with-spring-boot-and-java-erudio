...
public class PersonController {
    ...
    @Operation(summary = "Finds People by First Name", description = "Finds People by First Name",
            tags = {"People"},
            responses = {
                @ApiResponse(description = "Success", responseCode = "200",
                        content = {
    ...
    public ResponseEntity<PagedModel<EntityModel<PersonVO>>> findPersonsByName(
        ...
        var sortDirection = "desc".equalsIgnoreCase(direction) ? Sort.Direction.DESC : Sort.Direction.ASC;
        Pageable pageable = PageRequest.of(page, size, Sort.by(sortDirection, "firstName"));
        return ResponseEntity.ok(this.service.findPersonsByName(firstName, pageable));
    }
